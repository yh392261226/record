...
if [ $# -lt 1 ] || [ $# -gt 2 ]; then
  ...
  echo "Usage: $0 username [project (merp|soufeel ...)]";
  exit 1
fi
if [ "" != "$2" ]; then
  pmark=$2    #项目标识
else
  pmark=soufeel  #默认是soufeel的网站 magento代码
fi
newusername=$1
...
log_privalleg=777 																															#日志权限 方便用户使用
log_path=/logs                                                          				#日志目录
nginx_conf_path=/usr/local/nginx/conf/vhost                    									#nginx 配置文件位置
nginx_conf_source_path=/data/jst_server_shell/new_developer/nginx_conf     			#nginx 源配置文件位置
code_source=/data/jst_server_shell/new_developer/code_source/$pmark        			#程序代码文件位置
nginx_bin=/usr/local/nginx/sbin/nginx                                 					#nginx 执行程序位置
public_path=/data/public                                      									#共享目录
server_host=192.168.0.11                                                        #服务器ip地址
smbconf=/etc/samba/smb.conf                                                     #samba配置文件地址
alias_ext=com                                                                   #创建别名的后缀名
email_ext=company.com                                                           #git global的邮箱后缀
...
#fi
### 5
touch $web_path/${newusername}/error.log
touch $web_path/${newusername}/access.log
chmod $log_privalleg $web_path/${newusername}/error.log $web_path/${newusername}/access.log
if [ "$?" = "0" ]; then
...
if [ "$pmark" == "merp" ]; then
	sed -i -e "/include\ common\/location\/magento.conf;/ c\\
	include\ common\/location\/${pmark}.conf;" $nginx_conf_path/${newusername}.${alias_ext}/nginx_conf.conf;
fi
...
access_log $web_path/${newusername}/access.log;
error_log $web_path/${newusername}/error.log;
...
###生成ssh的id_rsa
ssh-keygen -t rsa -b 4096 -C "${newusername}@${email_ext}"
if [ "$?" = "0" ]; then
  echo "10:set ssh key of $newusername ok!";
fi

###生成一个新用户的git配置文件
cat << EOF >> $web_path/$newusername/.gitconfig
[user]
  name = $newusername
  email = ${newusername}@${email_ext}
EOF
if [ "$?" = "0" ]; then
  echo "11:create git config global for $newusername ok!";
fi
...
#echo "Id_rsa.pub info"
echo  "id_rsa: $(cat $web_path/${newusername}/.ssh/id_rsa.pub)"
echo "*************************************************************"
